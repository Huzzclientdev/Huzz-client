...
public final class MotionBlurRenderer {
    ...
    private static final int MAX_SHADER_SAMPLES = 100;
    ...
    public static void applyMotionBlur(RenderTickCounter tickCounter) {
    ...
    private static void applyMotionBlurInternal(MinecraftClient client, HuzzConfig config, float deltaTime) {
        ...
        float baseStrength = Math.max(0.0F, Math.min(1.0F, config.getMotionBlurFrames() / (float) HuzzConfig.MAX_MOTION_BLUR_FRAMES));
        float scaledStrength = baseStrength * getRefreshRateScale();
        updateBlurStrength(scaledStrength, deltaTime);
        ...
        int sampleAmount = Math.max(12, Math.min(MAX_SHADER_SAMPLES, 20 + config.getMotionBlurFrames() * 20));
    ...
    private static void updateBlurStrength(float targetStrength, float deltaTime) {
        float lerp = deltaTime <= 0.0F ? 1.0F : Math.min(1.0F, deltaTime * 12.0F);
