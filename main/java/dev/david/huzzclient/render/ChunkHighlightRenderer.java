...
public final class ChunkHighlightRenderer {
    ...
    public static void render(WorldRenderContext context) {
        ...
        List<BlockEspTracker.EspBlock> susBlocks = HuzzClient.getSusTracker().getHighlightedBlocks();
        List<SusTracker.SusChunk> susChunks = HuzzClient.getSusTracker().getHighlightedChunks();
        ...
        List<PlayerChunkLoadTracker.LoadedChunk> loadedChunks = HuzzClient.getPlayerChunkLoadTracker().getLoadedChunks();
        boolean selectedBlockEnabled = config.getBlockSelectionMode() != HuzzConfig.BlockSelectionMode.OFF;

        if (client.player == null || client.world == null
            || (blockEspBlocks.isEmpty() && storageEspBlocks.isEmpty() && susBlocks.isEmpty() && susChunks.isEmpty() && holeEspHoles.isEmpty() && bedrockHoleEspHoles.isEmpty() && loadedChunks.isEmpty()
        ...
        for (BlockEspTracker.EspBlock espBlock : susBlocks) {
            renderSusBlock(queue, matrices, tracerStartX, tracerStartY, tracerStartZ, espBlock);
        }

        if (config.isSusChunkFinderEnabled()) {
            for (SusTracker.SusChunk chunk : susChunks) {
                renderSusChunk(queue, matrices, client, tracerStartX, tracerStartY, tracerStartZ, chunk, config.isSusChunkFinderTracers());
            }
        }
        ...
        if (config.isPlayerChunkFinderEnabled()) {
            for (PlayerChunkLoadTracker.LoadedChunk chunk : loadedChunks) {
                renderLoadedChunk(queue, matrices, client, chunk);
            }
        }
    ...
    private static void renderSusBlock(
        RenderCommandQueue queue,
        MatrixStack matrices,
        double tracerStartX,
        double tracerStartY,
        ...
        queue.submitCustom(matrices, QUAD_LAYER, (entry, consumer) ->
            addBoxFill(consumer, entry, x1, y1, z1, x2, y2, z2, red, green, blue, 0.22F));
        queue.submitCustom(matrices, LINE_LAYER, (entry, consumer) -> {
            addBoxOutline(consumer, entry, x1, y1, z1, x2, y2, z2, red, green, blue);
            if (espBlock.tracers()) {
                addLine(consumer, entry, tracerStartX, tracerStartY, tracerStartZ, centerX, centerY, centerZ, red, green, blue);
    ...
    private static void renderLoadedChunk(
        RenderCommandQueue queue,
        MatrixStack matrices,
        MinecraftClient client,
        PlayerChunkLoadTracker.LoadedChunk chunk
    ) {
        int minX = chunk.chunkX() << 4;
        int minZ = chunk.chunkZ() << 4;
        int maxX = minX + 16;
        int maxZ = minZ + 16;
        double y = client.world == null ? client.player.getY() - 0.02D : client.world.getBottomY() + 0.04D;
        float[] color = loadTimeColor(chunk.loadTimeMs());
        queue.submitCustom(matrices, QUAD_LAYER, (entry, consumer) ->
            addQuad(consumer, entry, minX, y, minZ, maxX, y, minZ, maxX, y, maxZ, minX, y, maxZ, color[0], color[1], color[2], 0.28F));
    }

    private static void renderSusChunk(
        RenderCommandQueue queue,
        MatrixStack matrices,
        MinecraftClient client,
        double tracerStartX,
        ...
        int maxX = minX + 16;
        int maxZ = minZ + 16;
        double y = client.world == null ? client.player.getY() - 0.01D : client.world.getBottomY() + 0.08D;
        double centerX = minX + 8.0D;
        double centerZ = minZ + 8.0D;

        queue.submitCustom(matrices, QUAD_LAYER, (entry, consumer) ->
            addQuad(consumer, entry, minX, y, minZ, maxX, y, minZ, maxX, y, maxZ, minX, y, maxZ, 1.0F, 0.12F, 0.12F, 0.32F));
        queue.submitCustom(matrices, LINE_LAYER, (entry, consumer) -> {
            addLine(consumer, entry, minX, y + 0.01D, minZ, maxX, y + 0.01D, minZ, 1.0F, 0.12F, 0.12F);
            addLine(consumer, entry, maxX, y + 0.01D, minZ, maxX, y + 0.01D, maxZ, 1.0F, 0.12F, 0.12F);
            addLine(consumer, entry, maxX, y + 0.01D, maxZ, minX, y + 0.01D, maxZ, 1.0F, 0.12F, 0.12F);
            addLine(consumer, entry, minX, y + 0.01D, maxZ, minX, y + 0.01D, minZ, 1.0F, 0.12F, 0.12F);
            if (tracers) {
                addLine(consumer, entry, tracerStartX, tracerStartY, tracerStartZ, centerX, y, centerZ, 1.0F, 0.12F, 0.12F);
            }
        });
    }

    private static float[] loadTimeColor(long loadTimeMs) {
        if (loadTimeMs <= 250L) {
            return new float[]{0.1F, 0.9F, 0.35F};
        }
        if (loadTimeMs <= 1000L) {
            return new float[]{1.0F, 0.9F, 0.18F};
        }
        if (loadTimeMs <= 2500L) {
