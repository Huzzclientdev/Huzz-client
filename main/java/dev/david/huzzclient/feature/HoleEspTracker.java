...
public final class HoleEspTracker {
    ...
    private static List<Hole> scan(MinecraftClient client, int range) {
        ...
        LongSet usedCells = new LongOpenHashSet();
        ...
        int minY = Math.max(client.world.getBottomY(), centerY - 24);
        int maxY = Math.min(client.world.getTopYInclusive(), centerY + 8);
        ...
        for (int y = maxY; y >= minY; y--) {
            for (int x = centerX - range; x <= centerX + range; x++) {
                ...
                for (int z = centerZ - range; z <= centerZ + range; z++) {
                    ...
                    if (usedCells.contains(posLong)) {
                        continue;
                    }
                    double dy = y - centerY;
                    if (dxSquared + dy * dy + dz * dz > rangeSquared) {
                        continue;
                    }

                    mutablePos.set(x, y, z);
                    if (!isAir(world, mutablePos)) {
                        continue;
                    }

                    Hole lineHole = detectLineHole(world, mutablePos);
                    if (lineHole != null) {
                        results.add(lineHole);
                        addAllCells(usedCells, lineHole.cells());
                        continue;
                    }

                    Hole singleHole = detectSingleHole(world, mutablePos);
                    if (singleHole != null) {
                        results.add(singleHole);
                        addAllCells(usedCells, singleHole.cells());
                    }
                }
            }
        }
    ...
    private static void addAllCells(LongSet usedCells, Set<BlockPos> cells) {
