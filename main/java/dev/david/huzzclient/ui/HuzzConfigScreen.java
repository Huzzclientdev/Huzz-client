...
public final class HuzzConfigScreen extends Screen {
    ...
    public void render(DrawContext context, int mouseX, int mouseY, float delta) {
        ...
        for (int columnIndex = 0; columnIndex < COLUMNS.size(); columnIndex++) {
            ...
            drawColumnBody(context, layout, columnLayout, COLUMNS.get(columnIndex));
        }
        ...
        for (int columnIndex = 0; columnIndex < COLUMNS.size(); columnIndex++) {
            ColumnLayout columnLayout = layout.columns().get(columnIndex);
            drawColumnHeader(context, layout, columnLayout, COLUMNS.get(columnIndex));
        }
    }
    ...
    private void addModuleWidgets(ColumnLayout columnLayout, Module module, int moduleTop) {
        ...
        switch (module) {
            ...
            case PLAYER_NOTIFIER, PLAYER_ESP, AUTO_DISCONNECT, FAKE_MEDIA, FAKE_PAY, SILENT_SET_HOME, DUAL_ARM_ANIMATION, BEDROCK_PROTECT, AUTO_EAT, AUTO_MINE, LSD, NONE -> {
            }
    ...
    private void addHudWidgets(ColumnLayout columnLayout, int moduleTop) {
        ...
        addHudToggle(columnLayout, moduleTop, 4, workingCopy.isHudMusicEnabled(), () -> workingCopy.setHudMusicEnabled(!workingCopy.isHudMusicEnabled()));
        addHudToggle(columnLayout, moduleTop, 5, workingCopy.isHudMusicLargeUi(), () -> workingCopy.setHudMusicLargeUi(!workingCopy.isHudMusicLargeUi()));
        addHudToggle(columnLayout, moduleTop, 6, workingCopy.isHudMusicThemeEnabled(), () -> workingCopy.setHudMusicThemeEnabled(!workingCopy.isHudMusicThemeEnabled()));
        addStepButtons(columnLayout, moduleTop, 7, "<", ">", true,
    ...
    private void drawColumnHeader(DrawContext context, Layout layout, ColumnLayout columnLayout, ColumnGroup group) {
        HuzzUi.drawPanel(
            context,
            columnLayout.left(),
            layout.top(),
        ...
        Text groupTitle = Text.literal(group.title().toUpperCase(Locale.ROOT));
    ...
    private void drawModule(DrawContext context, ColumnLayout columnLayout, Module module, int top) {
        ...
        // Module cards use neutral rendering so music/theme accent does not tint their body.
        HuzzUi.drawRoundedRect(context, columnLayout.innerLeft() + 1, top + 2, columnLayout.innerRight() + 1, bottom + 2, HuzzUi.withAlpha(HuzzUi.PANEL_SHADOW, 72), 4);
        HuzzUi.drawRoundedRect(context, columnLayout.innerLeft(), top, columnLayout.innerRight(), bottom, HuzzUi.withAlpha(moduleBorderColor(module, accent), 210), 4);
        HuzzUi.drawRoundedRect(context, columnLayout.innerLeft() + 1, top + 1, columnLayout.innerRight() - 1, bottom - 1, HuzzUi.withAlpha(moduleFillColor(module), 228), 3);
        HuzzUi.drawHeaderBar(context, columnLayout.innerLeft() + 1, top + 1, columnLayout.innerRight() - 1, top + MODULE_HEADER_HEIGHT, HuzzUi.withAlpha(HEADER_GRAY_DARK, 236));
        ...
        switch (module) {
            ...
            case HUD -> {
                ...
                drawOptionRow(context, columnLayout, top, 4, "Music", workingCopy.isHudMusicEnabled() ? "Shown" : "Hidden", workingCopy.isHudMusicEnabled());
                drawOptionRow(context, columnLayout, top, 5, "Large", workingCopy.isHudMusicLargeUi() ? "On" : "Off", workingCopy.isHudMusicLargeUi());
                drawOptionRow(context, columnLayout, top, 6, "Theme", workingCopy.isHudMusicThemeEnabled() ? "On" : "Off", workingCopy.isHudMusicThemeEnabled());
                drawOptionRow(context, columnLayout, top, 7, "Src", workingCopy.getMusicHudSource().label(), true);
            }
            case CRAFTER_MACRO -> {
    ...
    private int optionCount(Module module) {
        int baseRows = switch (module) {
            ...
            case BREAK_PROGRESS, PLAYER_NOTIFIER, AUTO_DISCONNECT, PLAYER_ESP, AIM_FOV, SWING_SPEED, AUTO_REPLENISH,
                AUTO_BRIDGE, TUNNEL_BASE_FINDER, AUTO_WALK, AUTO_TOOL, AUTO_EAT, AUTO_MINE, FAKE_MEDIA, FAKE_PAY, LSD, FAST_USE, AUTO_TOTEM, AUTO_ROCKET, NAME_PROTECT,
    ...
    private int liveCount(Module module) {
        return switch (module) {
            ...
            case BREAK_PROGRESS, AIM_FOV, SWING_SPEED, FREE_CAM, FREE_LOOK, AUTO_REPLENISH, AUTO_BRIDGE, AIM_ASSIST, AUTO_TOOL, AUTO_CRYSTAL, FAST_USE,
                STASH_FINDER, CUSTOM_FOV, FULL_BRIGHT, MOTION_BLUR, TIME_CHANGER, NO_RENDER, BLOCK_SELECT, ITEM_NAMETAGS, FAKE_MEDIA, FAKE_PAY, FAKE_STATS, LSD, AUTO_TOTEM, AUTO_ROCKET,
                NAME_PROTECT, SKIN_PROTECT, AUTO_RECONNECT, FAKE_SPAWNER, FAKE_ELYTRA, BEDROCK_PROTECT, SILENT_SET_HOME, DOUBLE_CLICK, DUAL_ARM_ANIMATION, TUNNEL_BASE_FINDER, AUTO_EAT, AUTO_MINE, AUTO_WALK, HUD, NONE -> -1;
        };
    }

    private int accentColor(Module module) {
        if (workingCopy.isHudMusicThemeEnabled()) {
            return HuzzUi.uiAccentColor();
        }
        return switch (module) {
    ...
    private int moduleFillColor(Module module) {
        if (workingCopy.isHudMusicThemeEnabled()) {
            return HuzzUi.withAlpha(HuzzUi.uiAccentColor(), 166);
        }
    ...
    private int moduleBorderColor(Module module, int accent) {
        if (workingCopy.isHudMusicThemeEnabled()) {
            return HuzzUi.withAlpha(HuzzUi.uiAccentColor(), 255);
        }
