...
public final class HuzzConfigScreen extends Screen {
    ...
    private void addModuleWidgets(ColumnLayout columnLayout, Module module, int moduleTop) {
        ...
        switch (module) {
            ...
            case HOLE_ESP -> addHoleEspWidgets(columnLayout, moduleTop);
            case BREAK_PROGRESS, AUTO_REPLENISH -> {
            }
            ...
            case PRIME_CHUNK_FINDER, PLAYER_NOTIFIER, PLAYER_ESP, AUTO_DISCONNECT, FAKE_MEDIA, FAKE_PAY, NONE -> {
            }
    ...
    private void addHoleEspWidgets(ColumnLayout columnLayout, int moduleTop) {
        addStepButtons(columnLayout, moduleTop, 0, "-8", "+8", true,
            () -> workingCopy.setHoleEspRangeBlocks(workingCopy.getHoleEspRangeBlocks() - 8),
            () -> workingCopy.setHoleEspRangeBlocks(workingCopy.getHoleEspRangeBlocks() + 8));
    }
    ...
    private void addHudWidgets(ColumnLayout columnLayout, int moduleTop) {
        ...
        addDrawableChild(new HuzzActionButton(columnLayout.innerRight() - 32, top + 2, 28, 12,
            Text.literal(workingCopy.isFpsHudEnabled() ? "On" : "Off"),
            workingCopy.isFpsHudEnabled(),
            button -> {
                workingCopy.setFpsHudEnabled(!workingCopy.isFpsHudEnabled());
                rebuildScreen();
            }));
    }
    ...
    private void drawModule(DrawContext context, ColumnLayout columnLayout, Module module, int top) {
        ...
        switch (module) {
            ...
            case HOLE_ESP -> drawOptionRow(context, columnLayout, top, 0, "Range", workingCopy.getHoleEspRangeBlocks() + "b", workingCopy.isHoleEspEnabled());
    ...
    private int optionCount(Module module) {
        int baseRows = switch (module) {
            ...
            case HOLE_ESP, BREAK_PROGRESS, PLAYER_NOTIFIER, AUTO_DISCONNECT, PLAYER_ESP, SWING_SPEED, AUTO_REPLENISH,
                AUTO_BRIDGE, FAKE_MEDIA, FAKE_PAY, FAST_USE, AUTO_TOTEM, AUTO_ROCKET, NAME_PROTECT,
                SKIN_PROTECT -> 1;
            case STASH_FINDER -> 1;
    ...
    private boolean needsAutoKeybindRow(Module module) {
        return module != Module.NONE && module != Module.CHUNK_FINDER && module != Module.TIME_DEBUG
            && module != Module.BLOCK_ESP && module != Module.STORAGE_ESP && module != Module.FREE_CAM
    ...
    private HuzzConfig.ModuleKey moduleKey(Module module) {
        return switch (module) {
            case CHUNK_FINDER -> HuzzConfig.ModuleKey.CHUNK_FINDER;
            case TIME_DEBUG -> HuzzConfig.ModuleKey.TIME_DEBUG;
            case PRIME_CHUNK_FINDER -> HuzzConfig.ModuleKey.PRIME_CHUNK_FINDER;
    ...
    private int liveCount(Module module) {
        return switch (module) {
            ...
            case BREAK_PROGRESS, SWING_SPEED, FREE_CAM, FREE_LOOK, AUTO_REPLENISH, AUTO_BRIDGE, FAST_USE,
                STASH_FINDER, FULL_BRIGHT, MOTION_BLUR, BLOCK_SELECT, ITEM_NAMETAGS, FAKE_MEDIA, FAKE_PAY, FAKE_STATS, AUTO_TOTEM, AUTO_ROCKET,
                NAME_PROTECT, SKIN_PROTECT, AUTO_RECONNECT, HUD, NONE -> -1;
        };
    }
