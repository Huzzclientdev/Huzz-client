...
public final class HuzzUi {
    ...
    public static final int PANEL_OUTLINE = 0xFFB8B8C0;
    ...
    private static Integer themeAccentOverride;
    ...
    public static void drawHeaderBar(DrawContext context, int x1, int y1, int x2, int y2, int fillColor) {
        context.fill(x1, y1, x2, y2, blend(fillColor, withAlpha(themedAccent(), 255), 0.18F));
    }
    ...
    public static void setThemeAccentOverride(Integer accent) {
        themeAccentOverride = accent;
    }

    public static void clearThemeAccentOverride() {
    ...
    private static int themedAccent() {
        if (themeAccentOverride != null) {
            return themeAccentOverride;
        }
        try {
